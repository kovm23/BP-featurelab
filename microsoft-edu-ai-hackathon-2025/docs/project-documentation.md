...
```
backend/
├── app.py                      # Flask application factory, pipeline initialization
├── config.py                   # Konstanty (UPLOAD_FOLDER, CHECKPOINT_FOLDER, ...)
├── jobs.py                     # Thread-safe async job registry
├── pipeline/
...
Singleton objekt inicializovaný v `app.py` při startu serveru. Drží veškerý stav pipeline v paměti a persistuje na disk.
...
Pipeline stav se ukládá do `checkpoints/`:
