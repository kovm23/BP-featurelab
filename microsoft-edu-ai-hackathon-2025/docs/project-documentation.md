...
| 3. Training | Výběr cílového sloupce z CSV | Klasifikace: trénink RuleKit + RF + GBT, 3-way soft vote; Regrese: RuleKit. Vypočítají se metriky a CV | pravidla, metriky, feature importance |
...
### Stav implementace (aktualizace 2026-05-02)
...
- Pipeline podporuje `target_mode`:
  ...
  - `classification`: hybridní 3-way soft vote (RuleKit 1/3 + Random Forest 1/3 + Gradient Boosting 1/3); metriky `accuracy`, `balanced_accuracy`, `f1_macro`, `precision_macro`, `recall_macro`, `mcc`, `confusion_matrix`. RuleKit generuje interpretovatelná pravidla zobrazená v UI; RF + GBT zajišťují lepší predikční přesnost na malých datasetech.
...
| `model` | `RuleRegressor \| RuleClassifier \| None` | Natrénovaný RuleKit model (pravidla + predikce v regresi) |
| `xgb_model` | `dict{"rf": RF, "gbt": GBT} \| None` | Ensemble modelů pro klasifikaci: `rf` = RandomForestClassifier, `gbt` = GradientBoostingClassifier. `None` pro regresi nebo pokud trénink ensemble selhal. |
...
| `xgb_model.pkl` | Pickle | Scikit-learn ensemble: `{"rf": RandomForestClassifier, "gbt": GradientBoostingClassifier}`. Přítomno pouze po úspěšném tréninku klasifikace. |
...
- **RuleKit:** Normalizovaná frekvence features v pravidlech. Hodnota = podíl pravidel, v jejichž podmínkách se daná feature vyskytuje (rozsah 0–1, součet ≤ 1). Implementace: `_count_rule_features()` v `ml_rules.py`.
- **RF (pouze klasifikace):** Gini importance z Random Forestu, agregované zpět na původní feature názvy přes OHE sloupce. Implementace: `_rf_feature_importance()` v `ml_classification.py`.

Obě složky se vrací v API response (`feature_importance.rulekit`, `feature_importance.rf`) pro vizualizaci ve frontend.
...
Model pro klasifikaci — **3-way soft vote**:

| Model | Váha | Trénink |
|---|---|---|
| `RuleClassifier` (RuleKit, Java) | 1/3 | `RuleClassifier().fit(X_fit, y_fit)` |
| `RandomForestClassifier` (sklearn) | 1/3 | 200 stromů, `class_weight="balanced"` |
| `GradientBoostingClassifier` (sklearn) | 1/3 | 100 stromů, `max_depth=3`, `learning_rate=0.1` |

Finální predikce = průměr pravděpodobnostních distribucí (soft voting), zarovnaný na `label_classes`. Implementace: `_combined_predict_proba()` v `ml_classification.py`.

Fallback: pokud trénink ensemble selže, použije se samotný RuleKit.

Pravidla v UI pocházejí z RuleKitu (jediný z modelů který je generuje). Confidence zobrazené v UI odpovídají výstupu ensemble.

Cross-validace (`_run_cross_validation_ensemble()`):
- `StratifiedKFold` se zachováním rozložení tříd
- V každém foldu se trénují všechny tři modely zvlášť
- Metriky CV odpovídají blended predikci, ne pouze RuleKitu
...
- 20%: trénink RuleKit (pro pravidla)
- 45%: trénink RF + GBT ensemble (pouze klasifikace)
- 65%: výpočet predikcí (3-way blend nebo samotný RuleKit pro regresi)
- 78%: K-fold cross-validation (pouze klasifikace — trénuje všechny tři modely per fold)
- 98%: ukládání modelu/checkpointu
- 100%: dokončeno

Pro regresi zůstávají milníky beze změny (RuleKit only, bez ensemble krokü).
...
```json
{
  ...
  "feature_importance": {
    ...
    "rf":      {"action_intensity": 0.38, "speech_presence": 0.29, ...}  // pouze klasifikace
  },
  "warnings": [],
...
2. pokud je k dispozici ensemble (`xgb_model`), výsledný label vzniká z **3-way soft vote** (RuleKit 1/3 + RF 1/3 + GBT 1/3) přes `_combined_predict_proba()` s fallbackem na ensemble bez RuleKitu, pokud blend selže
3. pokud ensemble není k dispozici (fallback), použije se samotný `RuleClassifier`
4. confidence odpovídá maximální hodnotě v blended pravděpodobnostní distribuci
5. frontend zobrazuje použité RuleKit pravidlo (pokrývající rule z RuleKitu), i když samotná predikce pochází z blendu
6. pokud jsou k dispozici ground-truth labels, musí CSV obsahovat stejný target sloupec jako při tréninku; jinak backend vrátí chybu místo tichého fallbacku na jiný sloupec
7. pokud jsou k dispozici ground-truth labels, počítají se klasifikační metriky:
   - `accuracy`
...
- 25%: predikce (RuleKit + RF + GBT blended, nebo samotný RuleKit pro regresi)
...
3. Features se použijí pro ML model: v regresi RuleKit (interpretovatelný), v klasifikaci hybridní 3-way soft vote (RuleKit + Random Forest + Gradient Boosting)
...
**Regrese:** RuleKit (`RuleRegressor`) jako jediný model.
...
**Klasifikace:** Hybridní 3-way soft vote (RuleKit 1/3 + Random Forest 1/3 + Gradient Boosting 1/3).

Motivace pro ensemble v klasifikaci:
- RuleKit sám o sobě špatně generalizuje na malých nebo šumnatých datasetech (LLM-extrahované featury mají přirozený rozptyl)
- Random Forest a Gradient Boosting jsou odolnější vůči přetrénování a lépe si poradí s nerovnoměrným rozložením tříd (`class_weight="balanced"`)
- Interpretovatelnost je zachována přes RuleKit pravidla, která se zobrazují v UI bez ohledu na to, že samotná predikce pochází z blendu
...
- `_train_ensemble_classifier(X_fit, y_fit)` — trénuje `RandomForestClassifier` (200 stromů, `class_weight="balanced"`) a `GradientBoostingClassifier` (100 stromů, depth=3), vrátí `(rf, gbt)`
- `_ensemble_predict_proba(rf, gbt, X, label_classes)` — soft vote RF+GBT, zarovnaný na `label_classes`
- `_combined_predict_proba(rulekit, rf, gbt, X, label_classes, rulekit_weight=1/3)` — 3-way soft vote
- `_rf_feature_importance(rf, feature_columns, original_spec)` — Gini importance agregovaná zpět na pre-OHE feature názvy
- `_run_cross_validation_ensemble(X, y, label_classes, n_splits)` — `StratifiedKFold` trénující všechny tři modely per fold a evaluující blended predikci
